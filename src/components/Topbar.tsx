...
export function Topbar({ title, subtitle }: { title?: ReactNode; subtitle?: ReactNode }) {
  ...
  return (
    <header
      className={clsx(
        'drag-region relative h-12 shrink-0 flex items-center justify-between gap-6 pl-8 border-b border-border/70 bg-bg/40 backdrop-blur-md',
        isDesktop() ? 'pr-[148px]' : 'pr-8',
      )}
    >
