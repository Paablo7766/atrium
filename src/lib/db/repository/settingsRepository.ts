...
export function rowToSettings(row: SettingsRow, playbook: Settings['playbook']): Settings {
  ...
  return {
    ...
    lastCloudSyncAt: row.last_cloud_sync_at ?? undefined,
    lastSeenAppVersion: row.last_seen_app_version ?? undefined,
  }
}
...
export function saveSettings(db: Database.Database, settings: Settings): void {
  ...
  db.prepare(`
    INSERT INTO ${TABLES.settings} (
      ...
      tutorial_completed, demo_data, locale, cloud_sync_enabled, last_cloud_sync_at,
  ...
  `).run({
    ...
    cloud_sync_enabled: settings.cloudSyncEnabled ? 1 : 0,
    last_cloud_sync_at: settings.lastCloudSyncAt ?? null,
    last_seen_app_version: settings.lastSeenAppVersion ?? null,
  })
}
